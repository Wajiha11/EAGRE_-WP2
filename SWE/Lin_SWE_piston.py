...
settings = 1                                               # settings for wavemaker, 1 == original , 2 == yangs settings
alp = 0
dt = 0.02 
...
phi_half = fd.Function(V, name = "phi_half")                          # phi^n

eta_half = fd.Function(V, name = "eta_half")                         # phi^n
...
plt.title(" Wave frequency ($\omega$) vs. wave number (k)",fontsize=tsize)
plt.plot(k_plot, w_pot,  'k--',label = '$Potential$')
plt.plot(kp, sigma,  'ro')
plt.plot(k_plot, w_shallow , 'r--', label = '$Shallow$')
plt.xlabel('k ',fontsize=size)
plt.ylabel('$\omega $ ',fontsize=size)
plt.xlim([0.05, 3])
plt.legend(loc=1)
plt.grid()
# plt.show() 
...
if case == 1:
    ...
    print("Linear shallow water solved by using time-discrete VP based on SE scheme")
    ...
    print('Case 2: The linear SWE will be solved with time discrete VP based on SV scheme.')
