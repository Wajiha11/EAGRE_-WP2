...
Lx =  45 #26 # 33 # make it equal to wavelength 
Ly = 1
print("Lx =", Lx)
print('Ly =', Ly)
...
if case == 1:
    ...
    VP = ( fd.inner(X * Rht * h_new.dx(0), phi ) +  fd.inner (W * (h_new - h)/dt , phi ) \
        - fd.inner(phi_new , W*(h_new/dt)) - 1/2 * (Lw**2/W) * h_new * fd.inner(fd.grad(phi), fd.grad(phi))\
        - (1/2 * g * W *fd.inner(h_new,h_new)) + W*g*H0*h_new    ) * fd.dx \
        - (Lw * Rt*h_new*phi)*fd.ds(1) # VP after using short-hand notation
    
    ###### # VP without using short-hand notation
    ...
    #           - (1/2 * g * fd.inner(h_new,h_new))  + g*H0*h_new ) * (Lw - Rh)/Lw *fd.dx \
    #           - (Rt*h_new*phi)*fd.ds(1) # changed Rt to Rh 
    ...
    while (t <= t_end):
        ...
        if start_wavemaker  == 2:   # to stop the wavemaker as the time limit     
            if t > lim1:
                Rt.assign(0) 
                Rh.assign(0) 
                Rht.assign(0) 
