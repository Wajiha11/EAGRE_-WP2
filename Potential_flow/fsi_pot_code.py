...
# -*- coding: utf-8 -*-
"""
Created on Mon Jan  2 15:20:00 2023
...
coupling = False  # turn on coupling terms
...
## compute analytical initial phi and eta
    ...
    t0 = 0.0
    phi_exact_expr = D * cos(kx * x[0]) * cosh(kx * x[1]) * np.sin(omega * t0) # D cos(kx*x) cosh(kx*z) cos(omega t)
    eta_exact_expr = A * cos(kx * x[0]) * np.cos(omega * t0)
...
while t <= t_end + dt:
    ...
    output_data()
    EKin = assemble( 0.5*inner(grad(phi),grad(phi))*dx )
    EPot = assemble( 0.5*g*inner(eta,eta)*ds(top_id) )
    E = EKin+EPot
    ...
    if (t in t_plot):
        ...
        if ic ==0:
            # eta1vals = np.array([eta.at(x,  0.1) for x in xvals])
            # phi1vals = np.array([phi_f.at(x, 0.1) for x in xvals])            
            
            # ax1.plot(xvals, eta1vals, '--', label = f' $\eta_n: t = {t:.3f}$')
            # ax2.plot(xvals, phi1vals, '--', label = f' $\phi_n: t = {t:.3f}$')
            
            phi_exact_exprv = D * np.cos(kx * xvals) * np.cosh(kx * H0) * np.sin(omega * t) #
            eta_exact_exprv = A * np.cos(kx * xvals) * np.cos(omega * t)
    
            
            ax1.plot(xvals, eta_exact_exprv, ':', linewidth=1,label = f' $\eta_e: t = {t:.3f}$' ) 
            ax2.plot(xvals, phi_exact_exprv, ':', linewidth=1, label = f' $\eta_e: t = {t:.3f}$')
...
plt.figure(2)
plt.plot(time,Et,'.k', label = ' E_total')
plt.plot(time,Ep,'.b', label = ' E_pot')
plt.plot(time,Ek,'.r',  label = ' E_kin')
plt.xlabel('$t$')
plt.ylabel('$E(t)$')
