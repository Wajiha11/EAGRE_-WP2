...
# del_phi = fd.TestFunctions(mixed_V) # can we use del_phi in ds and dx?
...
outfile_phi = fd.File("results_pot_fexp/phi.pvd")
outfile_eta = fd.File("results_pot_fexp/eta.pvd")
...
LVP_phi_f = fd.LinearVariationalProblem(a_phi_f, L_phi_f, phif_new, bcs= BC_exclude_beyond_surface)
...
Koki said that I was missing the diagonal term (OL document (VP_testing) eq 5.16 g)and he told me to write: 
    
                                        a_01 = fd.dot( fd.grad(trial_eta), fd.grad(v_phi)) * fd.dx
                                        
But the code did not update the value of eta and then I swapped phi and eta by writing 

                                        a_01 = fd.dot( fd.grad(trial_phi), fd.grad(v_eta)) * fd.dx 
                                        
and then the value of eta started to update. Koki told me to recheck the mathematical formulation becaue the code worked when I swapped the indices
Although the code update the value of variables, it still explodes.

When Koki spotted missing a_01 term my L_eta equation was

                                        L_eta = eta * v_eta * fd.ds(top_id)
and

I had a_phi = fd.dot(fd.grad(trial_phi), fd.grad(v_phi)) * fd.dx  & a_eta = trial_eta * v_eta * fd.ds(top_id) on RHS, as follows

        LVP_eta_phi = fd.LinearVariationalProblem(a_eta + a_phi, L_eta, result_mixed  , bcs = [BC_phi_f, BC_exclude_beyond_surface_mixed])
        
So, I thought a_phi should be able to update the eta as I updated phi by using BC_phi_f = fd.DirichletBC(mixed_V.sub(1), phi_f, top_id) but
it did not as the diagonal term was missing.

'''

a_01 = fd.dot( fd.grad(trial_phi), fd.grad(v_eta)) * fd.dx # use del_phi instead of v_eta
# a_01 = fd.dot( fd.grad(trial_eta), fd.grad(v_phi)) * fd.dx
...
LVP_eta_phi = fd.LinearVariationalProblem(a_eta + a_phi - a_01, L_eta, result_mixed  , bcs = [ BC_exclude_beyond_surface_mixed, BC_phi_f])
# LVP_eta_phi = fd.LinearVariationalProblem(a_eta + a_phi, L_eta, result_mixed  , bcs = [BC_phi_f, BC_exclude_beyond_surface_mixed])
...
while t <= t_end + dt:
    ...
    # print('phi_f = ',phi_f.dat.data)
    
    # print('phi =', phi.dat.data)
